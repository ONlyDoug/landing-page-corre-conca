 ...
 * por padrão. @font-face com data URI embutida no SVG não funciona neste build do librsvg
 * (o texto sempre cai no fallback padrão, ignorando a fonte declarada). A solução que
 * funciona é registrar a pasta com as fontes via um fonts.conf customizado, gerado em /tmp
 * (único diretório com permissão de escrita na Lambda) e apontado pela env var
